...
```json
{
  ...
  "berlin_suggestion": "Berlin, Germany",
...
Typing `Berlin, Germany` produced a visible suggestion button with that exact
accessible name. Typing `Berlin` produced the Berlin suggestions listed above
and others, so the short input was ambiguous. Typing `Munich, Germany` produced
no matching suggestion during this observation; that result is not assumed to
hold across other sessions. Suggestion text also appeared in plain spans with
no explicit role. Text locators over the whole page can include locations in
search cards behind the picker; the suggestion button is the reliable target.
