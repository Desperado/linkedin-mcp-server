...
- **Connection Export**: Collect the authenticated user's connection usernames, names, and headlines with bounded scrolling
- **Contact Enrichment**: Extract visible profile and contact details in paced chunks, preserving raw section text as a fallback
...
| `PROXY_SERVER` | - | Route browser traffic through a proxy, as `scheme://host:port` (`http`, `https`, `socks4`, `socks5`), or with credentials as `http://user:pass@host:port`. Only the browser is routed, not the MCP transport. Inside a container `127.0.0.1` is the container itself, so a relay on the host is `host.docker.internal` (native Linux Docker also needs `--add-host=host.docker.internal:host-gateway`). A container on your own machine leaves through your address and needs no proxy. One on a cloud host or in another country does: take a dedicated static ISP address, or a sticky session from a residential pool, and create the session through it with `--login --login-viewer`. Details under [Using a proxy](https://github.com/stickerdaniel/linkedin-mcp-server#using-a-proxy). |
